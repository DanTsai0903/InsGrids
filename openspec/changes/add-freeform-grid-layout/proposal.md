...
- **Grid Presets**: Save and load grid configurations (dimensions, cell assignments, emoji placements)
...
- Grid preset management (save/load/delete)

### Technical Changes
- New `GridLayout` model to store grid configuration (rows, columns, cell assignments, emoji data)
- Extended `GridLayout` with `Codable` for preset support and auto-save
...
- Extension to existing `PresetManager` for grid preset support
- Auto-save mechanism using `UserDefaults` or document-based storage
